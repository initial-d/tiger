...
import codegen.C.exp.Num;
import codegen.C.type.Int;
...
public class TranslateVisitor implements ast.Visitor {
	...
	public void visit(ast.exp.ArraySelect e) {
		e.array.accept(this);
		codegen.C.exp.T ar = this.exp;
		e.index.accept(this);
		codegen.C.exp.T in = this.exp;
		this.exp = new codegen.C.exp.ArraySelect(ar, in);
		return;
	}
	...
	public void visit(ast.exp.Id e) {
		if(this.isField(e.id))
			this.exp = new codegen.C.exp.Id(e.id,true);
		else
			this.exp = new codegen.C.exp.Id(e.id,false);
		return;
	}

	private boolean isField(String id) {
		// TODO Auto-generated method stub
		ClassBinding cd = this.table.get(this.classId);
		Tuple tuple = new Tuple(this.classId,this.type , id);
		boolean b = (!this.tmpVars.contains(id)) && cd.fields.contains(tuple);
		return b;
	}
	...
	public void visit(ast.exp.Length e) {
		e.array.accept(this);
		codegen.C.exp.T n = this.exp;
		this.exp = new codegen.C.exp.Length(n);
	}
	...
	public void visit(ast.exp.NewIntArray e) {
		e.exp.accept(this);
		codegen.C.exp.T n = this.exp;
		this.exp = new codegen.C.exp.NewIntArray(n);
		return;
	}
	...
	public void visit(ast.exp.NewObject e) {
		...
		//this.type = new codegen.C.type.Class(e.id + "_vtable");
		//this.tmpVars.add(new codegen.C.dec.Dec(this.type, e.id+"_vtable_"));
		return;
	}
	...
	public void visit(ast.exp.Not e) {
		e.exp.accept(this);
		codegen.C.exp.T n = this.exp;
		this.exp = new codegen.C.exp.Not(n);
		return;
	}
	...
	@Override
	public void visit(ast.exp.True e) {  // @_@
	...
	public void visit(ast.stm.Assign s) {
		s.exp.accept(this);
		this.stm = new codegen.C.stm.Assign(s.id, this.exp , isField(s.id));
		return;
	}
	...
	public void visit(ast.stm.AssignArray s) {
		...
		codegen.C.exp.T in = this.exp;
		s.exp.accept(this);
		codegen.C.exp.T aa = this.exp;
		this.stm = new codegen.C.stm.AssignArray(s.id,in,aa , isField(s.id)) ;
		return;
	}
	...
	public void visit(ast.stm.While s) {
		...
		s.body.accept(this);
		codegen.C.stm.T thenn = this.stm;
		this.stm = new codegen.C.stm.While(condition, thenn);
		return;
	}

	// type
	@Override
	public void visit(ast.type.Boolean t) {  //@_@
